...
import {
  ...
  type SitePackSummary
} from './manifest.js';
import {
  parseInstructions,
  type SitePackInstructions
...
const sitePacksCache = new Map<string, Promise<SitePack[]>>();

export function clearSitePackCache(): void {
  sitePacksCache.clear();
}
...
export async function loadAllSitePacks(
  sitePacksRoot?: string
): Promise<SitePack[]> {
  const resolvedRoot = path.resolve(
    sitePacksRoot ?? (await getDefaultSitePacksRoot())
  );
  const cached = sitePacksCache.get(resolvedRoot);
  if (cached) {
    return cached;
  }
  ...
  sitePacksCache.set(resolvedRoot, loadPromise);

  try {
    return await loadPromise;
  } catch (error) {
    sitePacksCache.delete(resolvedRoot);
    throw error;
  }
}

export async function matchSitePackByUrl(
  url: string,
  sitePacksRoot?: string
): Promise<LoadedSitePack | null> {
  const hostname = new URL(url).hostname.toLowerCase();
  const packs = await loadAllSitePacks(sitePacksRoot);
  const matched = packs.find((pack) =>
    pack.manifest.domains.some(
      (domain) =>
  ...
  const matchedDomain =
    matched.manifest.domains.find(
      (domain) =>
        hostname === domain.toLowerCase() ||
        hostname.endsWith(`.${domain.toLowerCase()}`)
    ) ??
    matched.manifest.domains[0] ??
    hostname;
...
export async function loadSitePackById(
  siteId: string,
  sitePacksRoot?: string
): Promise<LoadedSitePack | null> {
  const packs = await loadAllSitePacks(sitePacksRoot);
  const matched = packs.find((pack) => pack.manifest.site_id === siteId);

  if (!matched) {
    return null;
  }

  const matchedDomain = matched.manifest.domains[0] ?? siteId;
