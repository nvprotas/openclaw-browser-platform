...
const defaultCamoufoxVenvPython = process.env.HOME
  ? path.join(
      process.env.HOME,
      '.openclaw',
      'venvs',
...
const browserRuntimeAvailable = camoufoxPythonCandidates.some((pythonBin) => {
  const result = spawnSync(pythonBin, ['-m', 'camoufox', 'version'], {
    stdio: 'ignore'
  });
...
<html>
  ...
  <body>
    <main>
      ...
      <button id="add-to-cart" onclick="document.querySelector('#cart-status').textContent='Added to cart'; document.querySelector('#go-cart').hidden = false; this.textContent='Added';">В корзину</button>
      <p id="cart-status">Not added</p>
      <a id="go-cart" href="/cart" hidden>Перейти в корзину</a>
    </main>
  </body>
</html>`);
      return;
...
<html>
  ...
  <body>
    <main>
      <h1>Your cart</h1>
      <p>Sample Book</p>
      <button onclick="location.href='/purchase/ppd/?order=local-litres-order&method=sbp&system=sbersbp'">Перейти к покупке</button>
    </main>
  </body>
</html>`);
      return;
    ...
    if (url.pathname === '/purchase/ppd' || url.pathname === '/purchase/ppd/') {
      const method = url.searchParams.get('method') ?? 'sbp';
      const system = url.searchParams.get('system') ?? 'sbersbp';
      response.end(`<!doctype html>
<html>
  <head><title>Оформление покупки</title></head>
  <body>
    <main>
      <h1>Оформление покупки</h1>
      <p>Способ оплаты</p>
      <p>method=${method}&system=${system}</p>
      <label for="payment-method-input-sbp">СБП</label>
      <input id="payment-method-input-sbp" type="radio" ${method === 'sbp' ? 'checked' : ''} />
      <label for="payment-method-input_russian_card" onclick="location.href='/purchase/ppd/?order=local-litres-order&method=russian_card&system=sbercard'">Российская карта</label>
      <input id="payment-method-input_russian_card" type="radio" ${method === 'russian_card' ? 'checked' : ''} />
      <button data-testid="paymentLayout__payment--button" onclick="document.querySelector('#payment-result').textContent='https://payecom.ru/pay_ru?orderId=local-order-1';">Продолжить</button>
  ...
  while (tempDirs.length > 0) {
    ...
    try {
      const raw = await readFile(
        path.join(dir, '.tmp/browser-platform/daemon.json'),
        'utf8'
      );
  ...
  const { stdout, stderr } = await execFileAsync(
    ...
    [cliPath, ...args],
    {
      ...
      env: {
        ...
        BROWSER_PLATFORM_TEST_LITRES_BASE_URL: serverUrl
      }
    }
  );
  return {
...
describe('browser-platform CLI + daemon runtime', () => {
  it.skipIf(!browserRuntimeAvailable)(
    ...
    async () => {
      const cwd = await mkdtemp(
        path.join(os.tmpdir(), 'browser-platform-test-')
      );
      tempDirs.push(cwd);
      ...
      expect(
        typeof (ensure.json?.daemon as { alreadyRunning?: unknown } | undefined)
          ?.alreadyRunning
      ).toBe('boolean');
      ...
      const openSession = open.json?.session as {
        sessionId: string;
      ...
      expect(open.json?.session).toMatchObject({
        ...
        status: 'open',
        trace: {
          tracePath: expect.stringContaining(
            path.join('.tmp', 'browser-platform', 'artifacts', 'traces')
          )
        },
        packContext: {
          matchedPack: false,
      ...
      expect(
        JSON.parse(await readFile(String(openSession.trace?.tracePath), 'utf8'))
      ).toMatchObject({
        sessionId,
      ...
      const context = await runCli(cwd, [
        'session',
        'context',
        '--session',
        sessionId,
      ...
      const observe = await runCli(cwd, [
        'session',
        'observe',
        '--session',
        sessionId,
      ...
      expect(
        (observe.json?.session as { visibleButtons: Array<{ text: string }> })
          .visibleButtons
      ).toEqual(
      ...
      expect(
        JSON.parse(
          await readFile(
            String(
              (observe.json?.session as { trace?: { tracePath: string } }).trace
                ?.tracePath
            ),
            'utf8'
          )
      ...
      const snapshot = await runCli(cwd, [
        'session',
        'snapshot',
        '--session',
        sessionId,
      ...
      const snapshotPayload = snapshot.json?.snapshot as {
        screenshotPath: string;
        htmlPath: string;
        trace?: { tracePath: string };
      };
      expect(snapshotPayload.screenshotPath).toContain(
        path.join('.tmp', 'browser-platform', 'artifacts', 'snapshots')
      );
      expect(snapshotPayload.htmlPath).toContain(
        path.join('.tmp', 'browser-platform', 'artifacts', 'snapshots')
      );
      expect(snapshotPayload.trace?.tracePath).toContain(
        path.join('.tmp', 'browser-platform', 'artifacts', 'traces')
      );
      expect(
        JSON.parse(
          await readFile(String(snapshotPayload.trace?.tracePath), 'utf8')
        )
      ).toMatchObject({
        sessionId,
  ...
  it.skipIf(!browserRuntimeAvailable)(
    'reuses provided storage state and reports auth state',
    async () => {
      const cwd = await mkdtemp(
        path.join(os.tmpdir(), 'browser-platform-test-')
      );
  ...
  it.skipIf(!browserRuntimeAvailable)(
    'detects login gate and reports it in auth state',
    async () => {
      const cwd = await mkdtemp(
        path.join(os.tmpdir(), 'browser-platform-test-')
      );
  ...
  it.skipIf(!browserRuntimeAvailable)(
    'runs a realistic action flow through session act',
    async () => {
      const cwd = await mkdtemp(
        path.join(os.tmpdir(), 'browser-platform-test-')
      );
      ...
      const sessionId = String(
        (open.json?.session as { sessionId: string }).sessionId
      );
      ...
      expect(
        JSON.parse(
          await readFile(
            String(
              (fill.json?.action as { trace?: { tracePath: string } }).trace
                ?.tracePath
            ),
            'utf8'
          )
      ...
      expect(addToCart.json).toMatchObject({
        ...
        action: {
          ...
          observations: expect.arrayContaining([
            expect.objectContaining({ code: 'PRODUCT_CTA_VISIBLE' })
          ])
        }
      });
  ...
  it.skipIf(!browserRuntimeAvailable)(
    'proves a LitRes-like search -> product -> add-to-cart -> cart flow with helpers',
    async () => {
      const cwd = await mkdtemp(
        path.join(os.tmpdir(), 'browser-platform-test-')
      );
      ...
      const submitTarget =
        searchPlan.submitTargets.find((target) => target.role === 'button') ??
        searchPlan.submitTargets[0];
      const submit = await runCli(cwd, [
        'session',
        'act',
        '--session',
      ...
      const observed = await runCli(cwd, [
        'session',
        'observe',
        '--session',
        sessionId,
      ...
      const resultTarget = chooseSearchResultTarget(
        observed.json?.session as SessionObservation,
        'Sample Book'
      );
      expect(resultTarget).toEqual({
        action: 'click',
        text: 'Sample Book Result'
      });
      ...
      const addToCartTarget =
        findAddToCartTargets(pack).find((target) => target.role === 'button') ??
        findAddToCartTargets(pack)[0];
      const addToCart = await runCli(cwd, [
        'session',
        'act',
        '--session',
      ...
      const openCartTarget =
        findOpenCartTargets(pack).find((target) => target.role === 'link') ??
        findOpenCartTargets(pack)[0];
      const openCart = await runCli(cwd, [
        'session',
        'act',
        '--session',
      ...
      expect(
        isCartVisible(
          (openCart.json?.action as { after: SessionObservation }).after
        )
      ).toBe(true);
      expect(openCart.json).toMatchObject({
        ok: true,
  ...
  it.skipIf(!browserRuntimeAvailable)(
    'proves a LitRes-like search -> results -> product flow with search helpers',
    async () => {
      const cwd = await mkdtemp(
        path.join(os.tmpdir(), 'browser-platform-test-')
      );
  ...
  it.skipIf(!browserRuntimeAvailable)(
    'runs local LitRes checkout scenario to hardStop finalPayload',
    async () => {
      const cwd = await mkdtemp(
        path.join(os.tmpdir(), 'browser-platform-test-')
      );
      ...
      expect(result.json).toMatchObject({
        ...
        stages: expect.arrayContaining([
          expect.objectContaining({
            step: 'load_site_pack_by_id',
            status: 'ok'
          }),
          expect.objectContaining({ step: 'run_scenario_total', status: 'ok' })
        ])
      });
    },
    30_000
