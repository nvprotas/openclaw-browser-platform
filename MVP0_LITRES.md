...
6. открыть корзину
7. дойти до checkout entry / payment boundary
8. если появляется SberPay URL, deeplink или payment form boundary — извлечь payment intent identifiers (`orderId`, `bankInvoiceId`, `mdOrder`, `formUrl` и связанные поля)
9. записать trace так, чтобы было понятно:
   - что произошло
...
-> add to cart
-> open cart
-> reach checkout entry / payment boundary
-> extract SberPay payment intent (if present)
...
- навигацию в корзину
- безопасную checkout/payment boundary без подтверждения оплаты
- traces
...
- reference implementation lives in `src/helpers/search.ts`
- demo entrypoint lives in `examples/demo-litres-search.ts`
...
## Commit 8.1 — Checkout boundary + SberPay payment-intent extraction

### Что сделать
Дотянуть flow до безопасной payment boundary и научиться извлекать SberPay identifiers без попытки подтвердить оплату.

### Для этого нужно
- helper/utility для детекта `payecom.ru/pay?...` и `platiecom.ru/deeplink?...`
- декодирование deeplink `params`, если они есть
- извлечение из URL / JSON / response body полей:
  - `orderId`
  - `bankInvoiceId`
  - `mdOrder`
  - `formUrl`
  - `merchantOrderNumber`
  - `merchantOrderId`
- structured JSON artifact, который можно вернуть агенту и положить в trace
- явный stop-point перед финальным payment submit

### Артефакты
- `src/helpers/sberpay.ts` или аналогичный extractor module
- обновления `site-packs/litres/checkout.md`
- при необходимости обновления `site-packs/litres/hints.json`
- `examples/demo-litres-sberpay.ts` или аналогичный smoke/demo script

### Done when
- flow доходит до checkout entry / payment boundary
- если появляется SberPay intent, ключевые идентификаторы извлекаются в structured JSON
- runtime останавливается до финального подтверждения оплаты
...
- JSON trace artifacts создаются на `session open/observe/act/snapshot`, а `snapshot` trace указывает на screenshot + HTML snapshot
- документы проекта явно фиксируют успешный LitRes pilot coverage и отдельный diagnostic/failure coverage без рискованных платёжных действий

---

## 6. Acceptance checklist

### Формальный статус Commit 10 / MVP0 acceptance

У Commit 10 теперь есть два явно зафиксированных acceptance-покрытия:

1. **Успешный LitRes pilot coverage**
   - подтверждён рабочий путь `home -> search -> product -> add_to_cart -> cart -> checkout/payment boundary`
   - подтверждён безопасный stop-condition до финального `Оплатить`
   - payment identifiers извлекаются в structured JSON без необратимого submit

2. **Diagnostic / failure coverage**
   - runtime пишет JSON trace artifacts на каждом ключевом шаге (`open`, `observe`, `act`, `snapshot`)
   - `session act` trace сохраняет `before/after`, diff и observations, так что видно, был ли `NO_OBVIOUS_CHANGE`, `PAYMENT_FLOW_STILL_ACTIVE`, `PAYMENT_IDS_DETECTED` и т.п.
   - `session snapshot` trace связывает диагностику с конкретными screenshot + HTML artifact paths

Это считается достаточным MVP0 acceptance уровнем без рискованных внешних шагов и без нажатия финальной оплаты.


### Техническая приёмка
- [x] daemon живёт отдельно от одного CLI-вызова
- [x] session state переживает несколько CLI-команд
- [x] весь CLI умеет возвращать JSON
- [x] traces создаются на каждом шаге (`session open/observe/act/snapshot`), а `snapshot` связывает trace с screenshot + HTML snapshot
- [x] pack `litres` автоматически матчится по домену

### Функциональная приёмка
- [x] LitRes home открывается
- [x] авторизованная сессия подхватывается, если доступна
- [x] поиск книги работает
- [x] карточка книги открывается
- [x] add-to-cart подтверждается
- [x] корзина открывается
- [x] checkout entry / payment boundary достигается без финального submit
- [x] если появляется SberPay intent, его идентификаторы извлекаются и сохраняются в trace / structured output

### Инженерная приёмка
- [x] по trace понятно, где именно упало
- [x] можно руками исправить `hints.json` / `instructions.md`
- [x] повторный запуск после правки pack даёт лучший результат
