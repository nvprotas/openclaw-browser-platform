...
- скачивает runtime через `python -m camoufox fetch` только если локальный cache Camoufox ещё не найден
...
Поведение fetch можно переопределить через `CAMOUFOX_FETCH=auto|always|never`. В режиме `auto` installer смотрит только на наличие локального runtime Camoufox и не пытается каждый раз перепроверять отдельные GeoIP-артефакты.
