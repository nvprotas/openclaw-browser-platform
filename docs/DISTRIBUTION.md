...
If Camoufox is not installed yet, rerun `install.sh` or provision it manually through a Python environment that can execute `python -m camoufox fetch`. The installer now uses `CAMOUFOX_FETCH=auto` by default and only fetches when the local Camoufox runtime cache is missing.
